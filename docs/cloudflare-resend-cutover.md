...
6. Create a scoped `CLOUDFLARE_API_TOKEN` that can deploy this Worker and store it, with the Cloudflare account ID, as GitHub Actions secrets. To upload Sentry source maps during the production build, add `SENTRY_AUTH_TOKEN` as a production environment secret and `SENTRY_ORG` and `SENTRY_PROJECT` as production environment variables. Source-map upload is intentionally disabled when `SENTRY_AUTH_TOKEN` is absent.
